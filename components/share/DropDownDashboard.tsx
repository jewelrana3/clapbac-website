    ...
    <div className="flex justify-between mb-5">
      ...
      <div>
        <Select value={status} onValueChange={handleStatusChange}>
          <SelectTrigger className="w-[180px]  border border-gray-300 rounded-md px-3 py-5 text-sm focus:outline-none">
            <SelectValue placeholder="All" />
          </SelectTrigger>
          <SelectContent className=" bg-white rounded-md shadow-md h-auto">
            <SelectGroup>
              {data.map((item, index) => (
                <SelectItem
                  key={index}
